...
# Example guidelines for testing
...
def get_matched_guidelines(
    ...
    # Add only customer messages to the session
    # The system will generate agent responses automatically
    async def _add_customer_messages_to_session() -> list[str]:
        ids: list[str] = []
        async with api.make_client() as client:
            for source, content in conversation_history:
                # Only add customer messages
            ...
            # After adding customer messages, get all events in the session
            ...
            # Return all event IDs from the session
...
def test_pricing_guideline_matching(
    ...
    # Create session
    _: Session = create_session(test_agent.id, test_customer.id)
    ...
    # Set up conversation history with a pricing query
    conversation_history: list[tuple[EventSource, str]] = [
    ...
    # Assert that only the pricing guideline is matched
